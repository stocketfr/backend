...
import type { TenantScope } from '../../platform/tenancy/tenant-query';
import type {
  ProductJoinRow,
  ProductQueryDto,
...
export const productBelongsToTenantCondition = (
  productIdExpression: SQL,
  tenantScope: TenantScope,
): SQL =>
  sql`${productIdExpression} IN (SELECT id FROM products WHERE ${tenantScope.whereTenant(products)})`;
