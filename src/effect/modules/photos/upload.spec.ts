...
describe('makePhotoUploadWorkflow', () => {
  ...
  it.effect('short-circuits source-key uploads when the source hash exists', () =>
    Effect.gen(function* () {
      const storage = makeInMemoryStorageAdapter();
  ...
  it.effect('deletes a just-uploaded object when idempotent create loses a race', () =>
    Effect.gen(function* () {
      const storage = makeInMemoryStorageAdapter();
      const repository = makeRepository({
        createIdempotent: () =>
