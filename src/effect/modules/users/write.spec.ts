...
describe('user write workflows', () => {
  it.effect('creates an auth user, tenant membership, roles, and response DTO', () =>
    Effect.gen(function* () {
      const calls: string[] = [];
      ...
      const result = yield* withTenantContext(workflows.createUser(createDto));
  ...
  it.effect('cleans up auth and local rows when local setup fails after auth create', () =>
    Effect.gen(function* () {
  ...
  it.effect('bans and unbans users through the local Better Auth repository', () =>
    Effect.gen(function* () {
  ...
  it.effect('deletes the auth user only after the final tenant membership is removed', () =>
    Effect.gen(function* () {
      const calls: string[] = [];
  ...
  it.effect('revokes Better Auth sessions after tenant membership and user checks', () =>
    Effect.gen(function* () {
      const calls: string[] = [];
      const workflows = makeUserWriteWorkflows({
        repository: makeRepository({
      ...
      expect(calls).toEqual(['member:user-1', 'auth:user-1', 'sessions:user-1']);
    }),
  );
});
